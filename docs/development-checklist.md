...
- ❌ 有 errors 必須修復，**禁止使用 eslint-disable 忽略**
- ✅ 只有 warnings 可以接受
...
**通過標準**: 無類型錯誤
- ✅ 應該無任何輸出（表示通過）
- ❌ 有錯誤輸出必須修復，**禁止使用 @ts-ignore/@ts-nocheck 新增忽略**
- 🔒 現有 @ts-nocheck 文件維持不變，新代碼必須嚴格檢查
...
#### 不可妥協原則
- ❌ **絕不躲避問題**：禁止使用 eslint-disable 或 @ts-ignore 來避開錯誤
- ❌ **絕不降低標準**：不修改配置來忽略新的錯誤  
- ❌ **絕不跳過檢查**：每個階段都必須通過檢查
- ✅ **實用優先**：修復問題本身，保持代碼品質
- 🔒 **現有忽略維持**：已有 @ts-nocheck 文件不變，新代碼嚴格檢查
