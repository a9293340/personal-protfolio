...
export const cardsData =
  CardDataConverter.convertAllProjects(projectsDataConfig);
...
export function getCardsByRarity(rarity) {
  return (
    cardsData.metadata.cardsByRarity[rarity]?.map(id => cardsData.cards[id]) ||
    []
  );
}
...
export function getCardsByAttribute(attribute) {
  return Object.values(cardsData.cards).filter(
...
export function openCardPack(packType = 'professional') {
  ...
  for (let i = drawnCards.length; i < cardCount; i++) {
    ...
    if (rand <= cardPackConfig.rarityProbability.legendary) {
      ...
      rand <=
      cardPackConfig.rarityProbability.legendary +
        cardPackConfig.rarityProbability.superRare
    ) {
