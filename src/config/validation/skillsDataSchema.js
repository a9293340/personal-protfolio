...
export class SkillsDataValidator {
  ...
  validateField(value, schema, location) {
    ...
    if (schema.enum && !schema.enum.includes(value)) {
      this.errors.push(
        `${location}: 值 "${value}" 不在允許的枚舉中: [${schema.enum.join(', ')}]`
      );
    }
    ...
    if (schema.type === 'string') {
      if (schema.minLength && value.length < schema.minLength) {
        this.errors.push(
          `${location}: 字符串長度 ${value.length} 小於最小長度 ${schema.minLength}`
        );
      }

      if (schema.maxLength && value.length > schema.maxLength) {
        this.errors.push(
          `${location}: 字符串長度 ${value.length} 大於最大長度 ${schema.maxLength}`
        );
      }
  ...
  validateCoordinateDistribution() {
    ...
    allNodes.forEach(node => {
      if (node.coordinates) {
        coordinates.push({
          ...
          distance: HexCoordinateValidator.calculateDistance(
            center,
            node.coordinates
          ),
        });
    ...
    if (uniqueCoords.size !== coordinates.length) {
      this.warnings.push(
        `發現 ${coordinates.length - uniqueCoords.size} 個重複座標`
      );
    }
  ...
  validateRingDistances() {
    ...
    if (this.config.tree.ring1) {
      this.config.tree.ring1.forEach(node => {
        const distance = HexCoordinateValidator.calculateDistance(
          center,
          node.coordinates
        );
        if (distance > 4) {
  ...
  validateCategoryBalance() {
    ...
    allNodes.forEach(node => {
      if (
        node.category &&
        Object.prototype.hasOwnProperty.call(categoryCount, node.category)
      ) {
        categoryCount[node.category]++;
      } else {
        this.errors.push(
          `技能 ${node.id} 的類別 ${node.category} 未在 categories 中定義`
        );
      }
    });
    ...
    if (maxCount - minCount > 2) {
      this.warnings.push(
        `技能類別不平衡: 最多 ${maxCount} 個節點，最少 ${minCount} 個節點`
      );
    }
  }
