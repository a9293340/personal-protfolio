...
export class DataAdapter extends BaseComponent {
    ...
    async loadProjectsData(projectsDataConfig) {
        try {
            ...
            console.log(`✅ DataAdapter: 成功載入並適配 ${adaptedProjects.length} 個專案`);
    ...
    parseTimelineData(timeline) {
        ...
        try {
            ...
            const endDate = timeline.endDate ? this.parseDate(timeline.endDate) : null;
    ...
    calculateDisplayPriority(project) {
        ...
        const featuredBonus = this.projectsData?.featured?.includes(project.id) ? 2 : 0;
    ...
    compareProjects(a, b) {
        ...
        switch (sortBy) {
            ...
            default:
                return (a.timeline.displayPriority - b.timeline.displayPriority) * order;
        }
    }
    ...
    applyFilters(filters = {}) {
        ...
        if (filters.dateRange && filters.dateRange.start && filters.dateRange.end) {
            const startPos = this.calculateTimelinePosition(this.parseDate(filters.dateRange.start));
            const endPos = this.calculateTimelinePosition(this.parseDate(filters.dateRange.end));
    ...
    getDataStatistics() {
        ...
        this.adaptedData.forEach(project => {
            ...
            // 稀有度統計
            stats.byRarity[project.visual.rarity] = (stats.byRarity[project.visual.rarity] || 0) + 1;
            ...
            stats.importanceStats.highest = Math.max(stats.importanceStats.highest, importance);
            stats.importanceStats.lowest = Math.min(stats.importanceStats.lowest, importance);
        });

        stats.importanceStats.average = Math.round((stats.importanceStats.average / this.adaptedData.length) * 100) / 100;
