...
export class TimelineLayoutEngine extends BaseComponent {
  ...
  calculateLayout(projects, viewport = {}) {
    ...
    console.log(
      `🧮 TimelineLayoutEngine: 開始計算 ${projects.length} 個專案的佈局`
    );
    ...
    console.log(
      `✅ TimelineLayoutEngine: 佈局計算完成，生成 ${layoutNodes.length} 個節點`
    );
  ...
  computeLayout(projects) {
    const _layout = this.config.layout;
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          const requiredDistance =
            (nodeA.nodeSize + nodeB.nodeSize) / 2 + minDistance;
  ...
  applyForce(node, force, damping) {
    // 限制垂直位移，保持時間軸的水平特性
    const maxVerticalDisplacement =
      this.config.layout.nodeSpacing.verticalRange * 0.1;
  ...
  applyImportanceBasedLayout(nodes) {
    ...
    // 按重要性排序
    const _sortedNodes = [...nodes].sort((a, b) => b.importance - a.importance);
    ...
    return nodes.map(node => {
      ...
      const verticalOffset =
        (1 - importance) * (viewport.height * 0.3) * (Math.random() - 0.5) * 2;
      node.position.y = centerY + verticalOffset;
  ...
  calculateConnectionPaths(nodes) {
    // 按時間順序排序
    const _sortedNodes = [...nodes].sort(
      (a, b) => a.timelinePosition.normalized - b.timelinePosition.normalized
    );
  ...
  getLayoutStatistics() {
    ...
    stats.averageIterations =
      Math.round((totalIterations / nodes.length) * 100) / 100;
