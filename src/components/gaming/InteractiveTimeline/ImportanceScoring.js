...
export class ImportanceScoring extends BaseComponent {
  ...
  calculateImportanceScore(project, context = {}) {
    ...
    console.log(
      `🧮 ImportanceScoring: 開始計算專案 ${project.id} 的重要性評分`
    );
    ...
    // 3. 稀有度加成
    const rarityAdjustedScore = this.applyRarityMultiplier(
      weightedScore,
      project.rarity
    );
    ...
    console.log(
      `✅ ImportanceScoring: 專案 ${project.id} 評分完成: ${finalScore}/10`
    );
    return result;
  }
  ...
  assessScalability(project, technologies) {
    ...
    if (
      techNames.some(
        tech => tech.includes('docker') || tech.includes('kubernetes')
      )
    ) {
  ...
  applyTemporalAdjustments(score, timeline) {
    ...
    const projectYear = timeline.coordinates
      ? timeline.coordinates.year
      : timeline.startDate
        ? parseInt(timeline.startDate.split('-')[0])
        : currentYear;
  ...
  applyCareerStageAdjustments(score, careerStage, project) {
    ...
    // 領導力加成（針對中級階段）
    if (
      adjustments.leadershipBonus &&
      project.stats &&
      project.stats.teamSize > 3
    ) {
  ...
  calibrateWithIndustryBenchmarks(score, category) {
    ...
    const calibrationFactor =
      this.state.calibrationFactors.industryAverage / industryAverage;
   ...
   */
  assessPortfolioValue(project, _stats) {
   ...
   */
  calculateRecognitionScore(project, _stats) {
    // 可見度評估
   ...
   * @returns {string} 優勢原因
   */
  getStrengthReason(dimension, _score) {
    const reasons = {
   ...
   * @returns {string} 改進建議
   */
  getImprovementSuggestion(dimension, _score) {
    const suggestions = {
  ...
  identifyKeyFactors(project, dimensionScores) {
    ...
    // 找出得分最高的維度
    const topDimension = Object.entries(dimensionScores).sort(
      ([, a], [, b]) => b - a
    )[0];
   ...
   */
  getDimensionImprovement(dimension, _project) {
    const improvements = {
