...
export class AnimationController extends BaseComponent {
  constructor(config = {}) {
    ...
    console.log(
      '🎮 [AnimationController] 動畫控制器初始化，配置:',
      this.config
    );
  }
    ...
    const safeInit = async (component, name) => {
      try {
        ...
        if (typeof component.init === 'function') {
          ...
          console.log(
            `✅ [AnimationController] ${name} 準備就緒（無需初始化）`
          );
        }
        return true;
      } catch (error) {
  ...
  async executePhase(phaseName) {
    ...
    try {
      ...
      console.error(
        `❌ [AnimationController] 階段 ${phaseName} 執行失敗:`,
        error
      );
  ...
  async executeParticleBurstPhase() {
    ...
    try {
      ...
      if (this.particleSystem.playBurst) {
        ...
        console.error(
          '❌ [AnimationController] ParticleSystem.playBurst 方法不存在'
        );
      }
    } catch (error) {
  ...
  async executeTransitionPhase() {
    ...
    await new Promise(resolve =>
      setTimeout(resolve, this.config.sequence.phases.transition.duration)
    );
  ...
  bindComponentEvents() {
    ...
    if (this.cardSummoning) {
      this.cardSummoning.on('summoningComplete', projectData => {
        console.log(
          '🎴 [AnimationController] 收到卡牌召喚完成事件:',
          projectData
        );
      });
    }
  ...
  updateProgress() {
    ...
    for (const [_phaseName, phase] of Object.entries(phases)) {
      if (
  ...
  handlePhaseError(phaseName, error) {
    console.error(
      `❌ [AnimationController] 階段 ${phaseName} 發生錯誤:`,
      error
    );
   ...
   * 錯誤恢復
   */
  recoverFromError(phaseName, _error) {
    console.log(
      `🔧 [AnimationController] 嘗試從階段 ${phaseName} 的錯誤中恢復`
    );
