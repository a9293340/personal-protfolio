...
export class ParticleSystem extends BaseComponent {
  constructor(config = {}) {
    ...
    if (!this.threeJsAvailable) {
      console.warn(
        '⚠️ [ParticleSystem] Three.js 不可用，粒子系統將使用降級模式'
      );
  ...
  async init() {
    ...
    try {
      ...
      if (this.config.container && this.config.container.appendChild) {
        ...
        console.warn(
          '⚠️ [ParticleSystem] 無效容器，添加到 body:',
  ...
  createRingFlowParticles() {
    ...
    const particleCount = Math.min(
      ringFlow.particleCount,
      this.config.performance.maxParticles * 0.6
    );
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const radius =
        ringFlow.radius + (Math.random() - 0.5) * ringFlow.ringWidth;
    ...
    geometry.setAttribute(
      'velocity',
      new window.THREE.Float32BufferAttribute(velocities, 3)
    );
    geometry.setAttribute(
      'phase',
      new window.THREE.Float32BufferAttribute(phases, 1)
    );
  ...
  createBurstParticles() {
    ...
    const particleCount = Math.min(
      burst.particleCount,
      this.config.performance.maxParticles * 0.4
    );
  ...
  updateRingFlowParticles(time) {
    ...
    const velocities =
      this.ringFlowParticles.geometry.attributes.velocity.array;
