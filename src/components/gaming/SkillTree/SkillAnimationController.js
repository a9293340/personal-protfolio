...
export class SkillAnimationController extends EventManager {
  ...
  bindMediaQueries() {
    ...
    mediaQuery.addListener((e) => {
      ...
      console.log(`SkillAnimationController: 減少動畫設置 = ${this.reducedMotion}`);
    });
  }
   ...
   * 處理技能狀態變化
   */
  handleSkillStatusChange({ skillId, oldStatus, newStatus, state }) {
    console.log(`SkillAnimationController: 處理狀態變化 ${skillId}: ${oldStatus} -> ${newStatus}`);
    ...
    // 執行動畫
    this.animateSkillStatusChange(skillElement, animationType, oldStatus, newStatus);
  }

  /**
   * 處理批量狀態更新
   */
  handleBatchStateUpdate({ rounds, states }) {
  ...
  animateSkillStatusChange(element, animationType, oldStatus, newStatus) {
    ...
    // 創建動畫時間軸
    const timeline = this.createAnimationTimeline(element, animationType, config, newStatus);
  ...
  fallbackToCSS(element, keyframes, options) {
    ...
    // 生成 CSS 規則
    const cssRule = this.generateCSSAnimation(animationClass, keyframes, options);
  ...
  clearAllAnimations() {
    // 停止所有運行中的動畫
    for (const [id, animation] of this.runningAnimations) {
