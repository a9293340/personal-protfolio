...
export class PoeStyleSkillTree extends EventManager {
  ...
  analyzeBranchStructure() {
    ...
    const branchCategories = {
      frontend: {
        ...
        angle: 0,
        skills: [],
        color: this.config.colors.frontend,
      },
      architecture: {
        name: '系統架構',
        angle: 60,
        skills: [],
        color: this.config.colors.architecture,
      },
      ai: {
        name: 'AI工程',
        angle: 120,
        skills: [],
        color: this.config.colors.ai,
      },
      devops: {
        name: 'DevOps',
        angle: 180,
        skills: [],
        color: this.config.colors.devops,
      },
      database: {
        name: '資料庫',
        angle: 240,
        skills: [],
        color: this.config.colors.database,
      },
      backend: {
        name: '後端開發',
        angle: 300,
        skills: [],
        color: this.config.colors.backend,
      },
    };
    ...
    this.branches.forEach((branch, categoryKey) => {
      ...
      branch.skills.forEach((skill, index) => {
        ...
        skill._renderInfo = {
          ...
          color: branchColor,
          branchIndex: index,
          pathConnections: this.calculateConnections(
            skill,
            index,
            branch.skills
          ),
        ...
        if (skill.skills && skill.skills.length > 0) {
          skill.skills.forEach((subSkill, subIndex) => {
            const subPosition = this.generateSubSkillPosition(
              x,
              y,
              subIndex,
              skill.skills.length,
  ...
  generateMainBranchPath(angle, skillCount) {
    ...
    const stepDistance =
      (maxDistance - baseDistance) / Math.max(skillCount - 1, 1);
    ...
    for (let i = 0; i < skillCount; i++) {
      ...
      const finalX =
        baseX +
        Math.cos(perpAngle) * (sideVariation + randomOffset) +
        Math.cos(angle + 0.3) * organicVariation;
      const finalY =
        baseY +
        Math.sin(perpAngle) * (sideVariation + randomOffset) +
        Math.sin(angle + 0.3) * organicVariation;
  ...
  renderConnections() {
    // 渲染主技能間的連接
    this.branches.forEach(branch => {
      branch.skills.forEach((skill, _index) => {
        ...
        connections.forEach(connection => {
          if (connection.type === 'center') {
            ...
            // 連接到分支中的前一個技能
            const targetSkill = branch.skills.find(
  ...
  setupDragAndZoom() {
    ...
    this.viewport.addEventListener('wheel', e => {
      ...
      const newScale = Math.max(
        0.3,
        Math.min(2, this.currentTransform.scale * delta)
      );
   ...
   */
  updateTransform() {
    this.skillTreeContainer.style.transform = `translate(${this.currentTransform.x}px, ${this.currentTransform.y}px) scale(${this.currentTransform.scale})`;
  }
