...
export class SkillTreeResponsiveAdapter extends EventManager {
  ...
  bindTouchEvents() {
    ...
    this.container.addEventListener(
      'touchstart',
      this.handleEnhancedTouchStart.bind(this),
      { passive: false }
    );
    this.container.addEventListener(
      'touchend',
      this.handleEnhancedTouchEnd.bind(this),
      { passive: false }
    );
  }
  ...
  handleDoubleTap(touch) {
    ...
    if (
      branchElement &&
      this.state.currentLayout === LayoutMode.VERTICAL_SCROLL
    ) {
  ...
  createVerticalBranchLayout(config, animate) {
    ...
    if (skillElements.length === 0) {
      console.warn(
        'SkillTreeResponsiveAdapter: 沒有找到技能元素，無法創建垂直佈局'
      );
      return;
    }
    ...
    branches.forEach((skills, branchName) => {
      ...
      if (!isCollapsed) {
        ...
        const skillsPerRow = Math.max(
          1,
          Math.floor((containerWidth - 40) / (config.skillSize + 10))
        );
  ...
  toggleBranchCollapse(branchName) {
    ...
    const isCurrentlyCollapsed =
      this.collapseState.collapsedBranches.has(branchName);
  ...
  expandBranch(branchName) {
    ...
    console.log(
      `SkillTreeResponsiveAdapter: 展開分支 "${branchName}" (${skills.length} 個技能)`
    );
  ...
  collapseBranch(branchName) {
    ...
    console.log(
      `SkillTreeResponsiveAdapter: 折疊分支 "${branchName}" (${skills.length} 個技能)`
    );
  ...
  updateCSSVariables(config) {
    ...
    root.style.setProperty(
      ...
      `${this.config.animations.layoutTransition}ms`
    );
  }
  ...
  handleResize() {
    ...
    if (newDevice !== this.state.currentDevice) {
      console.log(
        `SkillTreeResponsiveAdapter: 設備類型變化 ${this.state.currentDevice} -> ${newDevice}`
      );
  ...
  scanSkillElements() {
    ...
    console.log(
      `SkillTreeResponsiveAdapter: 掃描到 ${skillElements.length} 個技能元素`,
      {
  ...
  groupSkillsByBranch(skillElements) {
    ...
    skillElements.forEach(element => {
      const branch =
        element.dataset.branch || element.dataset.category || 'misc';
  ...
  getSkillsByBranch(branchName) {
    return Array.from(this.state.skillElements.values()).filter(
      element => element.dataset.branch === branchName
    );
  }
  ...
  updateContainerHeight(height) {
    if (this.state.currentLayout === LayoutMode.VERTICAL_SCROLL) {
      ...
      if (contentContainer) {
        ...
        console.log(
          `SkillTreeResponsiveAdapter: 設置內容容器高度為 ${height}px`
        );
      }
