...
export const DeviceDetector = {
  ...
  isTablet: () =>
    window.innerWidth >= ViewportConfig.breakpoints.mobile &&
...
export class SkillTreeViewportController extends EventManager {
  ...
  setupContainer() {
    ...
    this.contentContainer =
      this.container.querySelector('.skill-tree-content') ||
  ...
  updateDrag(clientX, clientY, timeStamp) {
    ...
    // 記錄原始位置
    const _oldX = this.viewport.x;
    const _oldY = this.viewport.y;
    ...
    // 應用邊界限制
    const clampedX = this.clampToBounds(
      newX,
      this.bounds.minX,
      this.bounds.maxX
    );
    const clampedY = this.clampToBounds(
      newY,
      this.bounds.minY,
      this.bounds.maxY
    );
  ...
  updatePinchZoom(touch1, touch2) {
    ...
    const scale =
      this.zoomState.startScale * (distance / this.zoomState.startDistance);
  ...
  clampZoom(scale) {
    return Math.max(
      ...
      Math.min(this.config.zoom.max, scale)
    );
  }
  ...
  translateViewport(deltaX, deltaY) {
    ...
    this.viewport.x = this.clampToBounds(
      newX,
      this.bounds.minX,
      this.bounds.maxX
    );
    this.viewport.y = this.clampToBounds(
      newY,
      this.bounds.minY,
      this.bounds.maxY
    );
  ...
  setInitialScale(scale, centerOnStart = true) {
    ...
    this.viewport.scale = Math.max(
      this.config.zoom.min,
      Math.min(scale, this.config.zoom.max)
    );
  ...
  animateToViewport(targetViewport) {
    ...
    const animate = currentTime => {
      ...
      // 插值計算
      this.viewport.x =
        startViewport.x + (targetViewport.x - startViewport.x) * eased;
      this.viewport.y =
        startViewport.y + (targetViewport.y - startViewport.y) * eased;
      this.viewport.scale =
        startViewport.scale +
        (targetViewport.scale - startViewport.scale) * eased;
