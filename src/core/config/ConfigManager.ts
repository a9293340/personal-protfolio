...
export class ConfigManager extends EventEmitter {
  ...
  private setupDefaultInterpolators(): void {
    ...
    this.interpolators.set('config', (match: string, path: string): string => {
      ...
      const result =
        keyPath.length > 0
          ? this.getNestedValue(config, keyPath.join('.'))
          : config;
    ...
    const envVars: EnvironmentVariables = {
      ...
      DEV:
        typeof globalThis !== 'undefined' && '__DEV__' in globalThis
          ...
          : true,
      PROD:
        typeof globalThis !== 'undefined' && '__PROD__' in globalThis
          ? (globalThis as any).__PROD__
          : false,
    };
  ...
  all(processInterpolation = true): Record<string, ConfigValue> {
    ...
    for (const [key, value] of this.configs) {
      result[key] = processInterpolation
  ...
  private interpolateString(str: string): string {
    return str.replace(/\{\{([^}]+)\}\}/g, (match, content) => {
      ...
      return variableInterpolator
        ? variableInterpolator(match, trimmed)
        : match;
    });
  }
  ...
  watch(
    keyOrCallback:
      | string
      | ((data: ConfigUpdateEvent | ConfigDeleteEvent) => void),
    callback?: (data: ConfigUpdateEvent | ConfigDeleteEvent) => void
  ): () => void {
  ...
  fromJSON(jsonStr: string, merge = false): ConfigManager {
    try {
      ...
      throw new Error(
        `Failed to import config from JSON: ${(error as Error).message}`
      );
    }
