...
export class EventManager {
  ...
  on(eventName, handler, options = {}) {
    ...
    if (listeners.length >= this.maxListeners) {
      console.warn(`EventManager: 事件 "${eventName}" 的監聽器數量超過限制 (${this.maxListeners})`);
      return;
    }
  ...
  emit(eventName, data = null) {
    ...
    for (let i = 0; i < listeners.length; i++) {
      ...
      try {
        ...
        console.error(`EventManager: 事件處理器執行錯誤 (${eventName}):`, error);
      }
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (timeout) {
        timeoutId = setTimeout(() => {
          ...
          reject(new Error(`EventManager: 等待事件 "${eventName}" 超時 (${timeout}ms)`));
        }, timeout);
      }
    });
  }
