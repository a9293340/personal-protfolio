...
4. **節點分布間距問題** - ✅ 最終修正 (2025-09-05 補充)
   - 節點分布從佔滿整條路徑(0%-100%)調整為緊密分布(5%-90%)
   - 修正公式：`progress = (index / (total - 1)) * 0.85 + 0.05`
   - 解決手機版時間軸線條與節點連接問題
   - 為未來config驅動的大量專案數據預做準備(支援彈性節點分布)
...
```javascript
// 原始分布 (問題)：佔滿整條路徑
const progress = total === 1 ? 0.5 : index / (total - 1); // 0%-100%

// 優化分布 (解決)：緊密分布在路徑中段
const progress = total === 1 ? 0.5 : (index / (total - 1)) * 0.85 + 0.05; // 5%-90%
```
...
- [x] 容器高度自適應，有充足底部空間
- [x] 年份篩選功能正常，但不顯示年份文字
- [x] 時間軸線條與所有節點完美連接 (節點分布算法優化)

## 重要開發筆記

### 未來擴展性考量
1. **Config驅動專案數據**
   - 當專案數據超過10個時，現有的節點分布算法已能彈性適應
   - `progress = (index / (total - 1)) * 0.85 + 0.05` 公式適用於任意數量的專案
   - 確保節點永遠分布在路徑的5%-90%區間，避免邊界截斷問題

2. **響應式設計原則**
   - 手機版使用緊密節點分布 + 簡化年份顯示
   - 桌面版保持完整功能 + 豐富視覺效果
   - 路徑端點計算考慮SVG viewBox邊界安全距離

---

**完成時間**: 2025-09-05  
**最終測試狀態**: ✅ 全部通過 (包含手機版線條連接)  
**版本狀態**: 準備提交版控